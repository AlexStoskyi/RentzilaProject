...
test('TC_390_Verify ""Назад"" button', async ({ page }) => {
  ...
  await expect(await photoPage.prevButton.innerText()).toBe(
    expectText.prevButtonText
  );
  await photoPage.clickPrevButton();
  await expect(await createUnitePage.categoryButton.innerText()).toContain(
    choseCategoryText
  );
  await expect(await createUnitePage.announcementTitleInput).toHaveValue(
    fakeName
  );
  await expect(
    await createUnitePage.manufacturerTransportEquipmentFiledText.innerText()
  ).toContain('ABC');
  await expect(await createUnitePage.mapLabelInput.innerText()).toContain(
    expectText.address
  );
});
